...
use App\ModelEcom\Currency;
use App\ModelEcom\BusinessSetting;
use App\ModelEcom\Product;
use App\ModelEcom\SubSubCategory;
use App\ModelEcom\FlashDealProduct;
use App\ModelEcom\FlashDeal;
use App\ModelEcom\OtpConfiguration;
use App\ModelEcom\Slider;
...
if (! function_exists('verified_sellers_id')) {
    function verified_sellers_id() {
        return App\ModelEcom\Seller::where('verification_status', 1)->get()->pluck('user_id')->toArray();
    }
}
...
if (! function_exists('convert_price')) {
    function convert_price($price)
    {
        ...
        $code = \App\ModelEcom\Currency::findOrFail(\App\ModelEcom\BusinessSetting::where('type', 'system_default_currency')->first()->value)->code;
...
if (! function_exists('home_discounted_price')) {
    function home_discounted_price($id)
    {
        ...
        $flash_deals = \App\ModelEcom\FlashDeal::where('status', 1)->get();
        $inFlashDeal = false;
...
function getShippingCost($index){
    ...
    if (\App\ModelEcom\BusinessSetting::where('type', 'shipping_type')->first()->value == 'flat_rate') {
        $calculate_shipping = \App\ModelEcom\BusinessSetting::where('type', 'flat_rate_shipping_cost')->first()->value;
    }
    elseif (\App\ModelEcom\BusinessSetting::where('type', 'shipping_type')->first()->value == 'seller_wise_shipping') {
        foreach (Session::get('cart') as $key => $cartItem) {
            $product = \App\ModelEcom\Product::find($cartItem['id']);
        ...
        if(!empty($admin_products)){
            $calculate_shipping = \App\ModelEcom\BusinessSetting::where('type', 'shipping_cost_admin')->first()->value;
        }
        if(!empty($seller_products)){
            foreach ($seller_products as $key => $seller_product) {
                $calculate_shipping += \App\ModelEcom\Shop::where('user_id', $key)->first()->shipping_cost;
            }
        }
    }
    ...
    if ($cartItem['shipping_type'] == 'home_delivery') {
        ...
        elseif (\App\ModelEcom\BusinessSetting::where('type', 'shipping_type')->first()->value == 'seller_wise_shipping') {
            if($product->added_by == 'admin'){
                return \App\ModelEcom\BusinessSetting::where('type', 'shipping_cost_admin')->first()->value/count($admin_products);
            }
            else {
                return \App\ModelEcom\Shop::where('user_id', $product->user_id)->first()->shipping_cost/count($seller_products[$product->user_id]);
            }
        }
        else{
            return \App\ModelEcom\Product::find($cartItem['id'])->shipping_cost;
        }
    }
    else{
        return 0;
...
if (! function_exists('isUnique')) {
    ...
    {
        $user = \App\ModelEcom\User::where('email', $email)->first();
